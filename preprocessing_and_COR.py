...
def draw_ROI (proj_0):
    rowmin,rowmax,colmin,colmax = neutompy.draw_ROI(proj_0, 'ROI from projection_0', ratio=0.85)
...
def outliers_filter (img_stack, radius_2D_neighborhood, threshold, axis=0, k=1.0, out=None):
    img_stack_filtered = ntp.remove_outliers_stack(img_stack,radius_2D_neighborhood,threshold,axis=0,outliers='bright',k=1)
    img_stack_filtered = ntp.remove_outliers_stack(img_stack,radius_2D_neighborhood,threshold,axis=0,outliers='dark',k=1)
    return img_stack_filtered

def find_center_of_rotation (proj_0, proj_180, nroi=None, ref_proj=None, ystep=5, ShowResults=True):
...
#def save_corrected_images (img_stack,new_path,digits):
